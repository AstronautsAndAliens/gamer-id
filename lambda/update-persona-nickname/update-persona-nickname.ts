...
export const handler: Handler = async (event:any, _context:any) => {
  ...
  const persona: IPersona = await client.db(process.env['MONGODB_DB_NAME']).collection(process.env['MONGODB_COLLECTION_PERSONA']).updateOne({gamer_id: gamer_id},{$set:{nickname: nickname}})
  console.log(persona)
  return {
    statusCode: 200,
    headers: {
